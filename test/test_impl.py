...
def test_for_each_word():
    ...
    cgen = CGen('main.c')
    cgen.append_line('#define LMC_GENERATE_SYMBOLS')    \
        .append_include('lmc.h', system_header=False)   \
    ...
    with cgen.with_open_function('int', 'main'):
        cgen.append_line('char words[{}];'.format(len(string) + 2)) \
            ...
            .append_line('lmc_for_each_word(iter, words) {')        \
    ...
    mgen.adjust('LDLIBS', '-lmockc -ldl', Mod.APPEND)
    mgen.generate()
